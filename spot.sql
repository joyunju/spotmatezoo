...
-- 시퀀스 생성
create sequence seq_detailOption_no
increment by 1 
start with 1
nocache;

-- comment
COMMENT ON TABLE detailOption IS '상세조건';

COMMENT ON COLUMN detailOption.no IS '상세조건번호';

COMMENT ON COLUMN detailOption.name IS '조건명';

CREATE UNIQUE INDEX PK_detailOption
	ON detailOption (
		no ASC
	);

ALTER TABLE detailOption
	ADD
		CONSTRAINT PK_detailOption
		PRIMARY KEY (
			no
		);
...
ALTER TABLE carDetail
	ADD
		CONSTRAINT FK_detailOption_TO_carDetail
		FOREIGN KEY (
			detailNo
		)
		REFERENCES detailOption (
			no
		);
...
CREATE TABLE spotmate (
	...
	carNo NUMBER, /* 차량번호 */
	startDate varchar(23), /* 출발날짜 */
	endDate varchar(10), /* 도착날짜 */
	people NUMBER, /* 인원수 */
...
---------------------- 스팟메이트 상세조건 ------------------------
DROP INDEX PK_spotmateDetail;

-- 테이블 삭제 : 스팟메이트상세조건
DROP TABLE spotmateDetail 
	CASCADE CONSTRAINTS;
...
-- 시퀀스 생성
create sequence seq_spotmateDetail_no
increment by 1 
start with 1
nocache;

-- comment
COMMENT ON TABLE spotmateDetail IS '스팟메이트상세조건';

COMMENT ON COLUMN spotmateDetail.no IS '메이트상세조건번호';

COMMENT ON COLUMN spotmateDetail.mateNo IS '메이트번호';

COMMENT ON COLUMN spotmateDetail.detailNo IS '상세조건번호';

CREATE UNIQUE INDEX PK_spotmateDetail
	ON spotmateDetail (
		no ASC
	);

ALTER TABLE spotmateDetail
	ADD
		CONSTRAINT PK_spotmateDetail
		PRIMARY KEY (
			no
		);
...
ALTER TABLE spotmateDetail
	ADD
		CONSTRAINT FK_spotmate_TO_spotmateDetail
		FOREIGN KEY (
			mateNo
		)
		REFERENCES spotmate (
...
ALTER TABLE spotmateDetail
	ADD
		CONSTRAINT FK_detailOption_TO_spotmateDetail
		FOREIGN KEY (
			detailNo
		)
		REFERENCES detailOption (
...
-- insert 생성
INSERT INTO spotmatedetail VALUES (
...
COMMENT ON COLUMN place.let IS '위도';
